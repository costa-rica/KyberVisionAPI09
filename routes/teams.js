...
const hasRights = (requiredRights) => {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (!groupContract) {
        return res.status(403).json({ message: 'Accès refusé : vous n\'êtes pas dans cette équipe.' });
      }

      if (!checkRights(groupContract.Rights_flags, requiredRights)) {
        return res.status(403).json({ message: 'Accès refusé : vous n\'avez pas les droits requis.' });
      }
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const rights = RIGHTS.VALIDATE_GROUP_REQUEST | RIGHTS.CREATE_REMOVE_PLAYER | RIGHTS.WRITE_ENABLED;
...
router.post('/add-user', authenticateToken, hasRights(RIGHTS.CREATE_REMOVE_PLAYER), async (req, res) => {
  try {
    ...
    const isPasswordValid = await bcrypt.compare(groupPassword, team.GroupPassword);
    if (!isPasswordValid) {
    ...
    await GroupContract.create({
      User_ID: userId,
      Team_ID: teamId,
      Rights_flags: RIGHTS.WRITE_ENABLED,
    });

    res.status(201).json({ message: 'Utilisateur ajouté à l\'équipe avec succès.' });
  } catch (error) {
...
router.put('/:teamId', authenticateToken, hasRights(RIGHTS.WRITE_ENABLED), async (req, res) => {
  try {
